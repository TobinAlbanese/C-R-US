  ...
  async function fetchEvents() {
      try {
          ...
          events = data.reduce((acc, event) => {
              if (event.date && event.time && event.service) {
  ...
  function renderCalendar(date) {
      ...
      for (let i = 0; i < totalCells; i++) {
          ...
          if (i >= firstDay) {
              ...
              console.log("Checking date:", fullDate, "Has event?", !!events[fullDate]);
              ...
              if (events[fullDate]) {
                  ...
                  eventText.textContent = events[fullDate].map(e => `${e.time} - ${e.service}`).join(", ");
